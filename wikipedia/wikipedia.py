...
class Wikipedia(commands.Cog):
    ...
    async def wikipedia(self, ctx: commands.Context, *, query: str):
        ...
        if not embeds:
            await ctx.send(
                error(f"Простите, не могу найти \"{query}\" на Википедии")
            )
        elif len(embeds) == 1:
    ...
    def generate_embed(page_json):
        ...
        if len(description) > 1000:
            ...
            description += f"... [(читать больше)]({url})"
        ...
        text = "Информация предоставлена Википедией"
        if timestamp:
            text += f"\nСтатья последний раз обновлена"
        embed.set_footer(
            text=text,
            icon_url=(
